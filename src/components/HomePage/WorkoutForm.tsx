...
// Modified to include category in each exercise
const exercisesData = {
  leg: [
...
export default function WorkoutForm({ setIsWorkoutDone }: { setIsWorkoutDone: (status: boolean) => void }) {
  ...
  const addCardioExercise = () => {
    const newExercise: CardioExercise = {
      ...
      time,
      date,
      ...(cardioType === 'treadmill' || cardioType === 'crossTrainer' ? { speed } : {}),
      ...(cardioType === 'uprightBike' || cardioType === 'crossTrainer' ? { distance } : {})
    };
  ...
  const saveCardioExercises = () => {
    console.log("Cardio exercises to save:", cardioExercises);
    setCardioExercises([]);
  };
  ...
  const saveWeightExercises = () => {
    console.log("Weight exercises to save:", weightExercises);
    setWeightExercises([]);
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    console.log("Complete workout data to save:", workoutData);
    ...
    <div className="flex flex-col items-center justify-center w-full min-h-screen py-8 px-4 bg-gray-900">
      <div className="w-full max-w-2xl bg-gray-800 rounded-xl shadow-lg p-6">
          ...
          <Tabs.Content value="cardio" className="outline-none">
            <div className="mb-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                ...
                {(cardioType === 'treadmill' || cardioType === 'crossTrainer') && (
                  <div>
                ...
                {(cardioType === 'uprightBike' || cardioType === 'crossTrainer') && (
                  <div>
            ...
            {cardioExercises.length > 0 && (
              <div className="mb-6 flex flex-col gap-2">
                ...
                <button
                  type="button"
                  onClick={saveCardioExercises}
                  className="flex items-center justify-center w-xs self-center gap-2 bg-teal-600 hover:bg-teal-700 text-white font-bold mt-4 py-2 px-4 rounded focus:outline-none focus:ring-2 focus:ring-teal-500 focus:ring-offset-2 focus:ring-offset-gray-800 transition-all"
                >
                  Save Cardio Sessions
                </button>
              </div>
            )}
          </Tabs.Content>
        ...
        <button
          ...
          disabled={(tab === 'cardio' && cardioExercises.length === 0) || 
                   (tab === 'weight' && weightExercises.length === 0)}
